...
| C-2 | **P0** | registration | No `fineTunableModels` entries; the backend can refuse to launch the Job | **answered from source**; block generated, entry pending Builder access |
...
Builder accepts a free-text Base Model value.

**The three open questions are now answered from source** (`fine_tunable_models.py`,
`pipeline_registry.py`, `on-prem` @ `e06d1a9`, cross-read against `main` @ `7ff6124` and
`testingjp1` @ `52e1d28`). Full contract in `MODEL_REGISTRATION.md`; the block itself is
generated by `scripts/build_registration.py` and gated by `tests/test_registration.py`.

**Is a non-image family accepted?** Yes, with no platform change.
`get_pipeline_fine_tunable_models` requires only a non-empty `id` and `baseWeights`; there is
no allow-list and the YOLO catalog is a fallback for an empty list, not a filter. The hazard
is that every other field carries a **silently substituted YOLO default** — `framework` and
`provider` become `"ultralytics"`, `supportedDatasetFormat` becomes `"yolo"`, and
`defaultTrainingParams` becomes YOLO's `COMMON_TRAINING_DEFAULTS`, whose learning rate is 50x
ours. Same failure mode as C-8: the substitution reaches the container in
`DIMER_MODEL_CONFIG_JSON` as though we had chosen it. An entry missing `id` or `baseWeights`
is skipped rather than rejected, so a typo silently removes a model from the dropdown.

**What does `baseWeights` mean?** An opaque identity string. It is required non-empty and used
for exactly one thing — as the key of `model_ids_by_base_weights`, which translates a legacy
`base_model` back into a `model_id`. It is never parsed, resolved to a path, or fetched.
`Qwen/Qwen3-1.7B@70d244cc…` is valid and is what we now emit, which makes the registration
self-describing about which commit is trained. Values must be unique across entries or the
lookup map remaps one model's legacy selector onto another's id.

**What `taskType` is accepted?** None of ours. `_normalize_task_type` has three outcomes —
`segmentation`, `image_classification`, and `object_detection` for **everything else** — so a
language-model pipeline is reported by the platform as `object_detection`. Adding an LM task
type is a backend change. It is inert for us: the normalized value feeds only the fallback
catalog, the default parameter schema, and `DIMER_PIPELINE_METADATA_JSON`, and neither
container reads that field — both bake `language_model_sft`. We register the honest value and
let the platform normalize it.

**A fourth answer nobody asked for, and the one that bites.** `resourceProfile` is
**advisory**: `_build_fine_tuning_container_resources` sizes the pod from cluster-wide
`WORKBENCH_FINE_TUNING_*` environment variables and never reads the registration. Per-model
sizing is therefore impossible — registering `"gpuCount": 1` does not get you a GPU. That is
decided by `WORKBENCH_FINE_TUNING_GPU_ENABLED`, one cluster-wide flag defaulting to `False`,
for every model in every pipeline. This makes C-4 sharper rather than softer, and it supplies
the early-failure signal C-4 needs: the backend injects `DIMER_EXPECTED_ACCELERATOR` as
