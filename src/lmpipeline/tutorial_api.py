...
def runtime_identity() -> dict[str, Any]:
    packages = {
        ...
        for name in TUTORIAL_RUNTIME_VERSIONS
    }
...
def assert_tutorial_runtime() -> dict[str, Any]:
    """Verify the exact lock without replacing the accelerator-coupled Torch wheel."""
    ...
    for name, expected in TUTORIAL_RUNTIME_VERSIONS.items():
        ...
        if comparable != expected:
            mismatches.append(
                f"{name}: {actual or 'missing'} (expected {expected})"
            )
    if mismatches:
        raise RuntimeError(
...
def assert_runtime_compatible(
    ...
    """Reject an artifact produced by a materially different tutorial software stack."""
    ...
    if missing:
        joined = ", ".join(missing)
        raise ValueError(
            f"Artifact provenance lacks runtime package versions: {joined}"
        )
    mismatches = [
        (
            f"{name}: artifact={producer_packages[name]} "
            f"runtime={consumer_packages.get(name)}"
        )
    ...
    if mismatches:
        raise ValueError(
            "Artifact/runtime compatibility mismatch:\n- "
            + "\n- ".join(mismatches)
        )
...
def resolve_artifact_model(provenance: dict[str, Any]) -> ModelEntry:
    """Resolve artifact provenance and require canonical model identity parity."""
    ...
    wrong_model = entry.model_id != provenance.get("baseModel")
    wrong_revision = entry.revision != provenance.get("baseModelRevision")
    if wrong_model or wrong_revision:
        raise ValueError(
            "Artifact base model identity/revision does not match the canonical registry"
        )
    return entry
...
def derive_validation_split(
    ...
    for example in examples:
        digest = hashlib.sha256(
            f"{seed}:{example.fingerprint()}".encode()
        ).digest()
        bucket = int.from_bytes(digest[:4], "big") % 10_000
        target = validation if bucket < threshold else train
...
def build_masked_example(
    ...
    *,
    ...
    if len(input_ids) > max_sequence_length:
        raise DatasetError(
            f"Line {example.line_number}: renders to {len(input_ids)} tokens, above "
            f"the configured limit of {max_sequence_length}.",
    ...
    for index, message in enumerate(messages):
        ...
        through_ids = _encode(tokenizer, through)
        prefix_stable = input_ids[: len(prefix_ids)] == prefix_ids
        turn_stable = input_ids[: len(through_ids)] == through_ids
        if not prefix_stable or not turn_stable:
            raise DatasetError(
        ...
        labels[len(prefix_ids) : len(through_ids)] = input_ids[
...
def load_base_model(
    ...
    *,
    ...
    model_dtype = (
        compute_dtype
        if method == "qlora"
        else (torch.bfloat16 if _bf16_supported() else torch.float32)
    )
    source = str(load_ref or entry.model_id)
    kwargs: dict[str, Any] = {
        "trust_remote_code": False,
    ...
    if loaded_revision and not local_files_only:
        if loaded_revision != entry.revision:
            raise ModelError(
    ...
    *,
    ...
    with torch.inference_mode():
        for batch in _batches(items, batch_size):
            ...
            supervised = int(
                (tensors["labels"] != IGNORE_INDEX).sum().item()
            )
...
def train_adapter(
    ...
    *,
    ...
    if torch.cuda.is_available():
        metrics.peak_gpu_memory_bytes = int(
...
def validate_prompt(
    ...
    *,
    ...
    """Validate user input before generation and return the rendered token count."""
...
def _safe_member_path(root: Path, member_name: str) -> Path:
    if "\\" in member_name:
        raise ValueError(
            f"Unsafe archive path (backslash): {member_name!r}"
        )
    ...
    with zipfile.ZipFile(zip_path) as archive:
        for info in archive.infolist():
            ...
            if canonical in seen and not info.is_dir():
                raise ValueError(
                    f"Duplicate archive member: {canonical}"
                )
            seen.add(canonical)
...
def verify_manifested_directory(
    ...
    *,
    ...
    wrong_format = manifest.get("format") != expected_format
    wrong_version = manifest.get("formatVersion") != expected_version
    if wrong_format or wrong_version:
    ...
    for record in manifest.get("files", []):
        ...
        if invalid:
            raise ValueError(
                f"SHA-256 or size mismatch: {record['path']}"
            )
    ...
    if listed_bytes != manifest.get("totalBytes"):
        raise ValueError("Manifest/file-set mismatch")
    required = {
        "adapter_config.json",
    ...
    if not required.issubset(listed):
        raise ValueError(
            "Artifact manifest is missing required adapter/provenance files"
        )
    return manifest
...
def export_adapter_bundle(
    ...
    *,
    ...
    model_card = (
        ...
        "Optimization metrics in metrics.json are tutorial evidence, "
        "not task-quality validation.\n"
    )
