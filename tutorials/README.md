...
[![Hugging Face](https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-HuggingFaceTB%2FSmolLM2--360M--Instruct-ffcc4d?style=flat)](https://huggingface.co/HuggingFaceTB/SmolLM2-360M-Instruct)

This directory holds the educational tutorials on **Supervised Fine-Tuning (SFT)** with **QLoRA** for a pinned
open causal language model: from digest-verified base weights and multi-turn conversational data to 4-bit
adapter training, before-and-after behavioral comparison, adapter packaging, and fresh reload verification.

Notebook specification: **DIMER Notebook Specification 2.0** — both notebooks are **standalone** (§4): they
are generated by `../tools/build_notebook.py` from `../tools/notebook_template.py` (E2E) and
`../tools/notebook_template_artifact_inference.py` (companion) and carry the repository's tutorial pipeline
module (`src/lmpipeline/pipeline.py`), the pinned base identity, the snapshot manifest and the runtime pins,
so the exported `.ipynb` keeps working without this repository being reachable. Do not edit the notebooks by
hand; edit the module or a template and regenerate (`python tools/build_notebook.py` and
`python tools/build_notebook.py --template tools/notebook_template_artifact_inference.py`; `--check` is
...
|---|---|---|---|---|---|---|---|---|---|
| `language_model_finetuning_colab.ipynb` | `E2E` | `GUIDED` | standalone (generated) | supervised QLoRA fine-tuning of the pinned `HuggingFaceTB/SmolLM2-360M-Instruct` snapshot on chat-formatted data: `validate_inputs` → input manifest (leakage and over-length rows rejected), assistant-only loss masking, deterministic baseline, readable QLoRA loop, `evaluation_report` → `sample-sanity` on the manufactured validation split (optimisation metrics, not task quality), adapter-off/on comparison, manifested adapter bundle export and fresh reload against the verified base | Colab T4 GPU (CUDA required for 4-bit training) | automatic | `train.jsonl` (+ optional validation/test) as files or one ZIP, gated off by default | unverified — no clean-runtime `Run all` execution recorded for this revision (REL1/REL8) | **Candidate** — static checks pass; the clean-runtime execution run is pending and will be recorded in `../docs/release-verification.md`, which must be reviewed for the exact notebook revision before promotion |
| `language_model_artifact_inference_colab.ipynb` | `ARTIFACT-INFERENCE` | `GUIDED` | standalone (generated) | verify an externally produced adapter bundle (safe extraction, per-file digests, provenance, pinned base identity) before any state is loaded; attach it to the digest-verified base with no network fallback; `validate_prompts` → input manifest; greedy adapter-off/on generation and sampled generation; `evaluation_report` → `not-measurable` (no labels) | CUDA recommended; CPU float32 works for a few prompts | automatic input prompts; **no automatic sample artifact yet** — default path uploads a bundle (§19 SART1/RUN2 gap) | bundle by upload or `ARTIFACT_DIR`; prompts typed into the form | unverified — no clean-runtime `Run all` execution recorded for this revision (REL1/REL8) | **Candidate** — static checks pass; the clean-runtime run (fed by the E2E bundle from a separate session) is pending and will be recorded in `../docs/release-verification.md` |
...
- **Standalone carrier (ST1–ST6):** the primary path performs no clone, repository install, or repository import. Section 2 of each notebook is `src/lmpipeline/pipeline.py` verbatim (tagged `metadata.dimer.embedded_module`; the only rewrite makes the default weights directory working-directory-relative), Section 3 carries the base identity and the 13-file SHA-256 manifest inline and asserts they agree with the module before anything is fetched, and Section 1 carries the exact `pyproject.toml` runtime pins. `metadata.dimer.generated_from` records the repository, the generating commit, the module's SHA-256 and the generator version.
- **Parity (PAR1–PAR4):** `tests/test_notebook_parity.py`, `tests/test_companion_parity.py` and `tools/validate_release_assets.py` fail when the carried module, the inline manifest or the inline pins differ from the repository, or when either notebook is not byte-identical to the generator's output.
- **Model acquisition:** `stage_missing_files(WEIGHTS_DIR, allow_download=True)` fetches only the manifest entries that are absent, from the Hugging Face Hub at the immutable revision `a10cc1512eabd3dde888204e902eca88bddb4951` (never `main`); `verify_snapshot` re-hashes every entry; `LanguageModelPipeline.from_pretrained(weights_dir=WEIGHTS_DIR)` loads the tokenizer and the base model from the verified files with `trust_remote_code=False` and `local_files_only=True`. No Hugging Face token is needed: the pinned model is public. The other `TUTORIAL_REGISTRY` entries (including the gated `llama-3.2-3b-instruct`) are carried for reference only and are not loadable from these notebooks.
- **Role stages:** `validate_inputs` (DAT24) writes `outputs/language_model_finetuning_input_manifest.json` with the schema, ceilings, per-split record and token counts, the dataset digest, the verdict and one recorded rejection finding (a leaked-split probe); `validate_prompts` writes `outputs/language_model_artifact_inference_input_manifest.json` for the companion (blank-prompt probe). `evaluation_report` (EVAL21) writes `outputs/<stem>_evaluation_report.json` — `sample-sanity` with `trainLoss` / `validationLoss` / `validationPerplexity` (the repository's own metric ids) on the manufactured validation split, `not-measurable` when no held-out split was scored, always naming what task-quality evidence would need.
- **Language-model fine-tuning obligations (Notebook Specification §20.7):** base model and immutable revision; chat template preserved between training and inference (the tokenizer travels in the bundle); which tokens receive loss is shown token by token; QLoRA (`nf4`, double quantization, LoRA rank/alpha) stated; deterministic greedy baseline recorded before training; optimisation metrics distinguished from task quality; base and adapted answers compared after training; adapter, tokenizer, metrics and provenance exported with an `artifact-manifest.json`; the adapter reloads against the verified base revision; adapters are stated to be incomplete without their base model.
- **Artifact-inference obligations (§18):** the bundle comes from outside the execution (upload or `ARTIFACT_DIR`); `extract_zip_safely` refuses path escapes, symlinks and oversized archives without `extractall`; `verify_artifact_bundle` validates format, digests, file set, `trustRemoteCode`, the 40-hex base revision and the pinned base identity before `PeftModel` is constructed; base weights are never taken from the bundle or the network; the trust boundary is stated; predictions are written machine-readably.
- `USE_BYOD` defaults to `False` so the E2E sample path never opens an upload dialog; the companion has no sample path by design (self-produced artifacts are not external-artifact evidence).
...
Every model in the registry enforces `trust_remote_code=False` to ensure safe weight instantiation. The standalone tutorials pin `smollm2-360m`; the rest of the table is carried in the module as `TUTORIAL_REGISTRY` for reference.

---

## Base-Model Acquisition

The standalone notebooks acquire exactly one base snapshot, `HuggingFaceTB/SmolLM2-360M-Instruct` at the
immutable revision `a10cc1512eabd3dde888204e902eca88bddb4951`: the inline `MANIFEST` (paths, byte
sizes, SHA-256 — the same `dimer-base-manifest.json` format the repository's `scripts/fetch_weights.py`
writes) is asserted against the carried module's identity, the missing entries are fetched from the
Hugging Face Hub at that revision, every file is re-hashed, and only then is the model loaded with
`trust_remote_code=False`. There is no branch or tag resolution, no offline-ZIP path and no token: the
pinned model is public. Gated registry entries (`llama-3.2-3b-instruct`) and the offline DIMER ZIP
packaging remain worker-side features documented in the repository README.
