...
Notebook specification: **DIMER Notebook Specification 1.1** — both notebooks are **standalone** (§3.6): they
...
|---|---|---|---|---|---|---|
| `language_model_finetuning_colab.ipynb` | `E2E` | standalone (generated) | supervised QLoRA fine-tuning of the pinned `HuggingFaceTB/SmolLM2-360M-Instruct` snapshot on chat-formatted data: `validate_inputs` → input manifest (leakage and over-length rows rejected), assistant-only loss masking, deterministic baseline, readable QLoRA loop, `evaluation_report` → `sample-sanity` on the manufactured validation split (optimisation metrics, not task quality), adapter-off/on comparison, manifested adapter bundle export and fresh reload against the verified base | Colab T4 GPU (CUDA required for 4-bit training) | `train.jsonl` (+ optional validation/test) as files or one ZIP, gated off by default | **Candidate** — static checks pass; the clean-runtime execution run is pending and will be recorded in `../docs/release-verification.md`, which must be reviewed for the exact notebook revision before promotion |
| `language_model_artifact_inference_colab.ipynb` | `ARTIFACT-INFERENCE` | standalone (generated) | verify an externally produced adapter bundle (safe extraction, per-file digests, provenance, pinned base identity) before any state is loaded; attach it to the digest-verified base with no network fallback; `validate_prompts` → input manifest; greedy adapter-off/on generation and sampled generation; `evaluation_report` → `not-measurable` (no labels) | CUDA recommended; CPU float32 works for a few prompts | bundle by upload or `ARTIFACT_DIR`; prompts typed into the form | **Candidate** — static checks pass; the clean-runtime run (fed by the E2E bundle from a separate session) is pending and will be recorded in `../docs/release-verification.md` |
